...
/// For higher layers (above 0 layers), select the location with the largest gradient
template<int pot>
inline int gridMaxSelection(Eigen::Vector3f* grads, bool* map_out, int w, int h, float THFac) {
  ...
  /// Traversing every other pot
  for(int y=1;y<h-pot;y+=pot) {
    for(int x=1;x<w-pot;x+=pot) {
      int bestXXID = -1;  /// gradx max
      int bestYYID = -1;  /// grady max
      int bestXYID = -1;  /// gradx-grady max
      int bestYXID = -1;  /// gradx+grady max

      float bestXX=0, bestYY=0, bestXY=0, bestYX=0;

      /// The starting point of the current grid
      Eigen::Vector3f* grads0 = grads+x+y*w;

      /// Find the top 4 best in the grid respectively
      for(int dx=0;dx<pot;dx++)
        for(int dy=0;dy<pot;dy++) {
          ...
          /// Threshold, why both are multiplied by 0.75
      ...
      /// Selected pixels are TRUE
      bool* map0 = map_out+x+y*w;

      /// Pick these largest pixels
      if(bestXXID>=0) {
        /// Not selected
        if(!map0[bestXXID])
          numGood++;
...
/// Same as above, just take pot as parameter
...
/// Grid size, select the largest one in the grid
inline int makePixelStatus(Eigen::Vector3f* grads,
                           bool* map, int w, int h,
                           ...
                           float THFac = 1)
{
  ...
  /// It is already the smallest, but the number is not enough, so reduce the threshold
  if(newSparsity==1 && sparsityFactor==1) THFac = 0.5;

  /// If the change in the grid size is small and the threshold is 0.5 || The number of points is within 20% of the error ||
  ...
  /// otherwise recursive
    ...
    // re-evaluate.
    sparsityFactor = newSparsity;
    /// \comment(edward): recursive function.
