...
typedef g2o::BlockSolver<g2o::BlockSolverTraits<8,1>> DSOBlock;

namespace dso {

/// constructor, apply for memory, initialize.
...
/// constructs the intrinsic parameter matrix, and some intermediate quantities.
/// TODO every class has this, it's better to use one directly.
...
/// use the inverse depth of the point projected on the current frame to genrate the inverse depth value of the point on each pyramid level.
...
void CoarseTracker::makeCoarseDepthL0(std::vector<FrameHessian*> frameHessians,
                                      ...
                                      CalibHessian Hcalib) {
  // make coarse tracking templates for latstRef.
  memset(idepth[0], 0, sizeof(float)*w[0]*h[0]);       /// 0 level.
  memset(weightSums[0], 0, sizeof(float)*w[0]*h[0]);
  ...
  /// STEP1: calculate the idepth weights and weighted idepths of other pixels on the 0 level of the latest frame projections.
  for(FrameHessian* fh : frameHessians) {
    for(PointHessian* ph : fh->pointHessians) {
      /// the last residul of the point is normal.
      /// the last time after optimization was set to 0 to indicate that the points were not deleted and the residuals were deleted.
      ...
      if(ph->lastResiduals[0].first != 0 && ph->lastResiduals[0].second == ResState::IN) {
        ...
        /// the residula of the point is good, the target of the last optimization is the ref.
  ...
  /// STEP2: generate idepth and weight from the lower level to the upper level.
  for(int lvl=1; lvl<pyrLevelsUsed; lvl++) {
    int lvlm1 = lvl-1;
    int wl = w[lvl], hl = h[lvl], wlm1 = w[lvlm1];
    ...
    for(int y=0;y<hl;y++) {
      for(int x=0;x<wl;x++) {
        int bidx = 2*x + 2*y*wlm1;
        /// ? why don;t you divide by 4?
        /// ! divide the weight by the sum of nice!
        idepth_l[x + y*wl] = idepth_lm[bidx] +
                             idepth_lm[bidx+1] +
                             idepth_lm[bidx+wlm1] +
                             idepth_lm[bidx+wlm1+1];
  ...
  /// STEP3: 0 and 1 levels for pixels without depth, fill them with four points inclined at 45 degrees around.
  // dilate idepth by 1.
  for(int lvl=0; lvl<2; lvl++) {
    int numIts = 1;
    ...
    for(int it=0; it<numIts; it++) {
      ...
      for(int i=w[lvl]; i<wh; i++) {
        if(weightSumsl_bak[i] <= 0) {
          /// use the points on four cornders to fill the ones without depth.
          /// bug: not very good for the points on the vertical edge, use the previous two lines to calculate.
          float sum=0, num=0, numn=0;
  ...
  /// STEP4: 2 levels up, for pixels with no depth, fill with four points up, down, left and right.
  // dilate idepth by 1 (2 on lower levels).
  for(int lvl=2; lvl<pyrLevelsUsed; lvl++) {
    int wh = w[lvl]*h[lvl]-w[lvl];
    int wl = w[lvl];
  ...
  /// STEP5: normalize the idepth of the point and assign it to the member variable pc_n
  // normalize idepths and weights.
  for(int lvl=0; lvl<pyrLevelsUsed; lvl++) {
    float* weightSumsl = weightSums[lvl];
    float* idepthl = idepth[lvl];
    ...
    /// ! pointer, just pass the pointer over, why do I always want to have a value.
    float* lpc_u = pc_u[lvl];
    float* lpc_v = pc_v[lvl];
    float* lpc_idepth = pc_idepth[lvl];
    float* lpc_color = pc_color[lvl];
    ...
    for(int y=2;y<hl-2;y++)
      for(int x=2;x<wl-2;x++) {
        ...
        /// after the evaluation, it becomes 1.
        weightSumsl[i] = 1;
      }
    pc_n[lvl] = lpc_n;
  }
...
/// for the residual b/w the  latest frame and the reference frame being tracked, find H and b.
void CoarseTracker::calcGSSSE(int lvl, Mat88 &H_out, Vec8 &b_out, SE3 refToNew, AffLight aff_g2l) {
  ...
  for(int i=0; i<n; i+=4) {
    ...
    acc.updateSSE_eighted(
        _mm_mul_ps(id,dx),  /// derivation of displacement x.
        _mm_mul_ps(id,dy),  /// derivation of displacement y.
        _mm_sub_ps(zero, _mm_mul_ps(id,_mm_add_ps(_mm_mul_ps(u,dx), _mm_mul_ps(v,dy)))),   /// derivation of displacement z.
        _mm_sub_ps(zero, _mm_add_ps(
            _mm_mul_ps(_mm_mul_ps(u,v),dx),
                _mm_mul_ps(dy,_mm_add_ps(one, _mm_mul_ps(v,v))))),  /// differentiate the rotation xi_1
        _mm_add_ps(
            _mm_mul_ps(_mm_mul_ps(u,v),dy),
            _mm_mul_ps(dx,_mm_add_ps(one, _mm_mul_ps(u,u)))),       /// derivate the rotation xi_2
        _mm_sub_ps(_mm_mul_ps(u,dy), _mm_mul_ps(v,dx)),             /// differential rotation xi_3
        _mm_mul_ps(a,_mm_sub_ps(b0, _mm_load_ps(buf_warped_refColor+i))),   /// derivate the target frame a.
        minusOne,                                                           /// derivate the target frame b.
        _mm_load_ps(buf_warped_residual+i),  /// residual.
        _mm_load_ps(buf_warped_weight+i));   /// huber weight.
  }
  ...
  H_out.block<8,3>(0,0) *= SCALE_XI_ROT;   /// bug: translation rotation order is wrong.
...
/// calculate the residual (energy value) obtained from the current pose projection, and perform some statistics.
/// construct as many poitns as possible to help track.
Vec6 CoarseTracker::calcRes(int lvl, SE3 refToNew, AffLight aff_g2l, float cutoffTH,
                            g2o::SparseOptimizer* optimizer,
                            VertexSE3PoseDSO* vtx_pose,
                            VertexPhotometricDSO* vtx_photo) {
  float E = 0;
  int numTermsInE = 0;
  ...
  int wl = w[lvl];
  int hl = h[lvl];
  Eigen::Vector3f* dINewl = newFrame->dIp[lvl]; // Rough estimate first
  float fxl = fx[lvl];
  float fyl = fy[lvl];
  float cxl = cx[lvl];
  float cyl = cy[lvl];
  ...
  /// this function will change the photometric params of the two frames before and after to two values.
  ...
  /// the point projected on the ref frame.
  int nl = pc_n[lvl];
  float* lpc_u = pc_u[lvl];
  float* lpc_v = pc_v[lvl];
  float* lpc_idepth = pc_idepth[lvl];
  ...
  for(int i=0;i<nl;i++) {
    ...
    float Ku = fxl * u + cxl;
    float Kv = fyl * v + cyl;
    /// depth on the current frame.
    float new_idepth = id/pt[2];

    /// every 32 points in level 0.
    if(lvl==0 && i%32==0) {
      ...
      /// count the moving size of pixels.
    ...
    /// image border check, skip if negative depth.
    ...
    /// X in reference frame.
    Vec3f Xref = Ki[lvl] * Vec3f(x,y,1) / id;

    EdgeSE3PosePhotoDSO* edge = new EdgeSE3PosePhotoDSO(Xref, dINewl, lvl, wl, hl, lastRef->ab_exposure, newFrame->ab_exposure, lastRef_aff_g2l, nl);
    edge->setVertex(0, vtx_pose);
    edge->setVertex(1, vtx_photo);
    edge->setMeasurement(lpc_color[i]);
    edge->setInformation(Eigen::Matrix<double,1,1>::Identity());
    edge->setId(numTermsInE);
  ...
  Vec6 rs;
  rs[0] = E;                                            /// projected energy value.
  rs[1] = numTermsInE;                                  /// number of points projected.
  rs[2] = sumSquaredShiftT/(sumSquaredShiftNum+0.1);    /// the size of the average pixel movement during pure translation.
  rs[3] = 0;
  rs[4] = sumSquaredShiftRT/(sumSquaredShiftNum+0.1);   /// the size of the avaerage pixel movement during translation + rotation.
  rs[5] = numSaturated / (float)numTermsInE;            /// % greater than curoff threshold.
...
/// set the optimized latest frame as the reference frame.
void CoarseTracker::setCoarseTrackingRef(std::vector<FrameHessian*> frameHessians,
                                         FrameHessian* fh_right,
                                         CalibHessian Hcalib) {
  assert(frameHessians.size()>0);

  /// \comment(edward): set last Reference Keyframe into 'lastRef'. It uses prior Keyframe SE3 pose.
  lastRef = frameHessians.back();

  // std::cout << "lastRef\n" << lastRef->shell->camToWorld.matrix() << std::endl;

  /// generate estimated idepth.
...
/// track new frames, optimize poses and photometric parameters.
                                      ...
                                      IOWrap::Output3DWrapper* wrap)
{
  ...
  algorithm->setUserLambdaInit(0.01);
  ...
  for(int lvl=coarsestLvl; lvl>=0; lvl--) {
    ...
    /// STEP1: calculate the residual error, gurantee that at most 60% of the residual error is greater than the threshold, calculate the normal equation.
    Vec6 resOld = calcRes(lvl, refToNew_current, aff_g2l_current, setting_coarseCutoffTH*levelCutoffRepeat, optimizer, vtx_pose, vtx_photo);

    //// ensure that the point greater than the threshold is less than 60%
    // while(resOld[5] > 0.6 && levelCutoffRepeat < 50) {
    //   /// if it exceeds the threshold, the threshold will be re-calculated
    //   levelCutoffRepeat*=2;
    ...
    optimizer->initializeOptimization(lvl);
    optimizer->setVerbose(false);
    std::cout << "[*] LV " << lvl << ", optimizing... " << std::endl;
    optimizer->optimize(maxIterations[lvl]);
    for(int iteration=0; iteration < maxIterations[lvl]; iteration++) {
      /// STEP2.1: calculate the increment.
      // Mat88 Hl = H;
      ...
      // /// the lambda is too small, and a factor is given to the increment. why???
      // float extrapFac = 1;
      ...
      // /// STEP2.2: after using incremental update, re-calculate the energy value.
      ...
      // /// accept if the average energy value is small.
      ...
      // /// STEP2.3: accept the normal equation, continue iterating, and optimize until the increment is small enough.
      // if(accept) {
    ...
    /// STEP3: record the last residual, optical flow indication, if the threshold is adjusted, re-calculate this level.
    // set last residual for that level, as well as flow indicators.
    // lastResiduals[lvl] = sqrtf((float)(resOld[0] / resOld[1]));
    lastResiduals[lvl] = sqrtf((float)optimizer->activeRobustChi2() / optimizer->edges().size());
  ...
  // set!
  lastToNew_out = vtx_pose->estimate();
  aff_g2l_out = vtx_photo->estimate();
  // lastToNew_out = refToNew_current;
  // aff_g2l_out = aff_g2l_current;

  /// STEP4: determine optimization failure.
    ...
    for(int y=3; y<h[lvl]-3; y++)
      for(int x=3; x<wl-3; x++) {
        ...
        if(bp[0] > 0 || nid >= 3) {
          ...
          /// \comment(edward): set colored point circle.
...
CoarseDistanceMap::CoarseDistanceMap(int ww, int hh) {
  /// calculated on the 0 level, so divide by 4.
...
/// for all current map point projections, generate distance field maps.
void CoarseDistanceMap::makeDistanceMap(std::vector<FrameHessian*> frameHessians,
                                        FrameHessian* frame)
{
  /// ? why use the 1 level.
  int w1 = w[1];
  ...
  for(FrameHessian* fh : frameHessians) {
    ...
    /// 0 level to 1 level transformation.
...
void CoarseDistanceMap::growDistBFS(int bfsNum) {
  ...
  for(int k=1;k<40;k++) {
    ...
    /// every time I look aroound the previous point.
    std::swap<Eigen::Vector2i*>(bfsList1,bfsList2);  /// swap each iteration.
...
/// generate distance field near point (u,v)
