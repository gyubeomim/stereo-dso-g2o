...
/// Returns the derivative value of the inverse depth
...
/// Transform points on host to target
...
/// Project the host frame to a new frame, and you can set the pixel offset dxdy, get:
/// parameter: [drescale newer than the old inverse depth] [uv new normalized plane]
/// [kukv new pixel plane] [KliP old normalized plane] [new_idepth new inverse depth]
EIGEN_STRONG_INLINE bool projectPoint(
    const float &u_pt,const float &v_pt,
    const float &idepth,
...
{
  /// Normalized plane points on host
  ...
  /// Target frame inverse depth than host frame inverse depth
  drescale = 1.0f/ptp[2];
  // printf("drescale is %f", drescale);

  /// Inverse depth on new frame
