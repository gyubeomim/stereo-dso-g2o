...
inline Vec2 affFromTo(Vec2 from, Vec2 to) {
...
struct FrameHessian {
  ...
  inline void setState(Vec10 state) {
    ...
    //// 8,9 terms are always 0.
    state_scaled[8] = SCALE_A * state[8];
    state_scaled[9] = SCALE_B * state[9];

#if 0
    std::cout << "state[6]: " << state[6] << ", state_scaled[6]: " << state_scaled[6] << std::endl;
    std::cout << "state[7]: " << state[7] << ", state_scaled[7]: " << state_scaled[7] << std::endl;
#endif
