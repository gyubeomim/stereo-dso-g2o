...
template<typename T> inline void deleteOut(std::vector<T*> &v, const int i) {
  /// Delete the memory pointed to by the i-th element
  delete v[i];

  /// Use the last one to fill in i
  v[i] = v.back();
...
/// Delete the i-th element, followed by the order. For ordered sequences
...
/// For ordered sequences, delete the elements of element
...
/// Check if there are infinite elements in the matrix, output msg and the matrix
...
class FullSystem {
 public:
  ...
  bool isLost;
  bool initFailed;
  bool initialized;         /// Whether to complete initialization
 ...
 private:
  /// Create global assignment, you can use sharedptr
  CalibHessian Hcalib;
  ...
  /// tracking thread lock
  boost::mutex trackMutex;

  /// All historical frames
  std::vector<FrameShell*> allFrameHistory;
  CoarseInitializer* coarseInitializer;

  /// The average chi2 last tracked
  Vec5 lastCoarseRMSE;
  ...
  /// Residuals of newly added activation points
  std::vector<PointFrameResidual*> activeResiduals;

  /// Threshold of activation point
  float currentMinActDist;

  std::vector<FrameHessian*> frameHessiansRight;

  /// All residual values on the current nearest frame
 ...
 public:
  /// \comment(edward): semi-direct method.
  SE3 Twc_prior_;
  SE3 Twc_posterior_;
  void UpdateTwcPrior(SE3 prior) { Twc_prior_ = prior; }
};
