...
class AccumulatedSCHessianSSE
{
 public:
  ...
  {
    /// If the number of frames changes
    if(n != nframes[tid])
    {
  ...
  /// Pose and intrinsic parameters Schur on idepth
  AccumulatorXX<8,CPARS>* accE[NUM_THREADS];

  /// B * Schur with idepth
  AccumulatorX<8>* accEB[NUM_THREADS];

  /// Schur with two positions for idepth
  AccumulatorXX<8,8>* accD[NUM_THREADS];

  /// Intrinsic parameters Schur on idepth
  AccumulatorXX<CPARS,CPARS> accHcc[NUM_THREADS];

  /// Intrinsic parameters b * Schur on idepth
