...
/// Extracting data from PointFrameResidual
void EFResidual::takeDataF()
{
  ...
  //// JpJdF[0~5] = J_dxi * (J_dI)^2 * J_didepth
  for(int i=0; i<6; i++) {
    JpJdF[i] = J->Jpdxi[0][i]*JI_JI_Jd[0] + J->Jpdxi[1][i]*JI_JI_Jd[1];
  }

  //// JpJdF[6,7] = J_dab * J_dI * J_didepth
  JpJdF.segment<2>(6) = J->JabJIdx * J->Jpdd;
}

/// Extracting data from FrameHessian
void EFFrame::takeData()
{
  /// Obtain a priori state, mainly photometric affine transformation
  prior = data->getPrior().head<8>();

  /// Difference between state and FEJ zero state
  delta = data->get_state_minus_stateZero().head<8>();

  /// The difference between the state and the prior
  /// ? But the prior is 0?
...
/// Read prior and current state information from PointHessian
void EFPoint::takeData()
{
  ...
  /// // TODO updates the linearization point every time. Isn't this always zero? ?
  /// Current state inverse depth minus linearization
  deltaF = data->idepth - data->idepth_zero;
}

/// Calculate the linearized updated residuals,
/// ! No square is called residual, square is energy
void EFResidual::fixLinearizationF(EnergyFunctional* ef)
{
  /// Get hostIDX-> targetIDX status increment
