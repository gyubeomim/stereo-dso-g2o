...
void AccumulatedSCHessianSSE::addPoint(EFPoint* p, bool shiftPriorToZero, int tid)
{
  ...
  /// hessian + marginalization to get hessian + transcendental hessian
  // TODO marginalized prior is different from normal prior
  ...
  /// Cross term of idepth and intrinsic parameters.
  VecCf Hcd = p->Hcd_accAF + p->Hcd_accLF;

  /// schur complement
  /// H_pi = Hcd * Hdd_inv * Hcd^T
...
/// Get the hessian matrix Schur complement from the accumulator
void AccumulatedSCHessianSSE::stitchDoubleInternal(MatXX* H, VecX* b,
                                                   ...
                                                   int min, int max, Vec10* stats, int tid)
{
  ...
  for(int k=min;k<max;k++) {
    ...
    /// Residual Schur of pose, photometric part
  ...
  if(min==0)
  {
    for(int tid2=0;tid2 < toAggregate;tid2++)
    {
      ...
      /// Residual Schur of intrinsic paramters Part
...
/// Calculate Schur H b for a single thread
