...
struct RawResidualJacobian
{
  ...
  /// 8 residuals per patch
  EIGEN_ALIGN16 VecNRf resF;

  // the two rows of d[x,y]/d[xi].
  EIGEN_ALIGN16 Vec6f Jpdxi[2];			// 2x6 /// Point-to-pose

  // the two rows of d[x,y]/d[C].
  EIGEN_ALIGN16 VecCf Jpdc[2];			// 2x4 /// Point-to-camera-params

  // the two rows of d[x,y]/d[idepth].
  EIGEN_ALIGN16 Vec2f Jpdd;			// 2x1 /// Point-to-inverse-depth

  // the two columns of d[r]/d[x,y].
  EIGEN_ALIGN16 VecNRf JIdx[2];			// 8x2 /// Patch photometric error point, 8 × 2

  // = the two columns of d[r] / d[ab]
  EIGEN_ALIGN16 VecNRf JabF[2];			// 8x2  /// Patch photometric error on photometric affine, 8x2


  /// Corresponding small hessian
