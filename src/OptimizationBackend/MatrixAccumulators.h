...
class AccumulatorXX {
 public:
  ...
  inline void initialize() {
    A.setZero();
    A1k.setZero();   //// k means one thousand
    A1m.setZero();   //// m menas one milloin
  ...
  inline void update(Eigen::Matrix<float,i,1> L,
                     Eigen::Matrix<float,j,1> R,
                     float w) {
    A += w*L*R.transpose();
    numIn1++;
    shiftUp(false);
...
class Accumulator11 {
 ...
 private:
  ...
  /// carry
  void shiftUp(bool force) {
    /// Greater than 1000, add up to k
    if(numIn1 > 1000 || force) {
      /// ? Why is it necessary to carry 1000 times?
      /// ! Answer: As long as it does not exceed 128 bits, an approximate number, the addition of 1000 32 bits, must not exceed
...
class Accumulator14 {
 public:
  ...
  inline void initialize()
  {
    ...
    /// (14 * 14-14) / 2 + 14 hessian only needs to calculate half
...
class AccumulatorApprox {
 public:
  ...
  /// [x, y] are 10-dimensional vectors, [a, c] are diagonals of common terms, and [b] are cross terms of common terms
...
class Accumulator9 {
 public:
  ...
  inline void initialize()
  {
    ...
    /// 128-bit, 16-byte alignment, so each number is stored in 4 floats
  ...
  inline void finish()
  {
    ...
    /// H matrix is symmetric, with only 45 values
    for(int r=0; r<9; r++) {
      for(int c=r; c<9; c++) {
        float d = SSEData1m[idx+0] + SSEData1m[idx+1] + SSEData1m[idx+2] + SSEData1m[idx+3];
        H(r,c) = H(c,r) = d;
  ...
  /// Multiply a 9-dimensional vector to get a 9 * 9 matrix
  inline void updateSSE(
      const __m128 J0,const __m128 J1,
      const __m128 J2,const __m128 J3,
      const __m128 J4,const __m128 J5,
      ...
      const __m128 J8)
  {
    /// A total of 45 values
    float* pt=SSEData;

    /// 9 values in the first line
    _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(J0,J0))); pt+=4;
    _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(J0,J1))); pt+=4;
    _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(J0,J2))); pt+=4;
    _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(J0,J3))); pt+=4;
    ...
    /// 8 in the second line, because of symmetry
    _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(J1,J1))); pt+=4;
    _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(J1,J2))); pt+=4;
    _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(J1,J3))); pt+=4;
    _mm_store_ps(pt, _mm_add_ps(_mm_load_ps(pt),_mm_mul_ps(J1,J4))); pt+=4;
  ...
  /// Weighted 9-dimensional vector to get 9 * 9 matrix
  ...
  /// Do not use _m128 for calculation
  inline void updateSingle(
      const float J0,const float J1,
      const float J2,const float J3,
      const float J4,const float J5,
  ...
  /// Not using alignment acceleration, weighted
