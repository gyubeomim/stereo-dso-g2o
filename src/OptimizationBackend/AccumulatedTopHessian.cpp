...
/// Calculate Hessian and Jres for the residuals
...
void AccumulatedTopHessianSSE::addPoint(EFPoint* p, EnergyFunctional const * const ef, int tid)
{
  ...
  /// Traverse all the residuals at that point
  for(EFResidual* r : p->residualsAll) {
    /// This is not the same as the running mode.
    /// Calculate only newly added residuals
    if(mode==0) {
      if(r->isLinearized || !r->isActive()) {
        continue;
      }
    }
    /// bug: This condition has been met. Calculate the old residual, calculated before.
    if(mode==1) {
      if(!r->isLinearized || !r->isActive()) {
        continue;
      }
    }
    /// The case of marginal computing
    if(mode==2) {
      if(!r->isActive()) {
        continue;
      }
    ...
    /// ID to control variables between different frames, distinguish the same two frames, but the host target roles are interchangeable
    int htIDX = r->hostIDX + r->targetIDX*nframes[tid];

    /// pose(6) + photometric a b(2)
    ...
    //// linearize mode
    if(mode==1) {
      /// Because the calculation is old, the update needs to be recalculated
      // compute Jp*delta
      ...
      for(int i=0; i<patternNum; i+=4) {
        ...
        /// Update b value linearly, marginalization amount, each time res_toZeroF
    ...
    /// Calculate hessian 10 * 10 matrix, [pose(6) + intrinsic parameters(4)]
    ...
    /// Calculate a 3 * 3 matrix, [photometric a(1), photometric b(1), residual r(1)]
    ...
    /// Calculate 10 * 3 matrix, [pose(6) + intrinsic parameters(4)] * [photometric a(1), photometric b(1), residual r(1)]
    ...
    //// J_dr_dp2^T * J_dr_didepth1
    Vec2f Ji2_Jpdd = rJ->JIdx2 * rJ->Jpdd;

    /// Residuals * idepth J
    //// J_dr_didepth1^T * r [scalar]
    bd_acc +=  JI_r[0]*rJ->Jpdd[0] + JI_r[1]*rJ->Jpdd[1];

    /// Photometric versus idepth hessian
    //// J_dr_didepth1^T * J_dr_didepth1 [scalar]
    Hdd_acc += Ji2_Jpdd.dot(rJ->Jpdd);

    /// Photometric versus intrinsic parameters J * Photometric versus idepth J
    //// J_dr_dC^T * J_dr_didepth1 [4x4]
...
/// H and b calculations on a single thread, or without using multiple threads
...
/// Constructing a Hessian matrix, b = Jres matrix
void AccumulatedTopHessianSSE::stitchDoubleInternal(MatXX* H, VecX* b,
                                                    EnergyFunctional const * const EF, bool usePrior,
                                                    int min, int max, Vec10* stats, int tid)
{
  int toAggregate = NUM_THREADS;

  /// No multi-threading, why can't you unify
  ...
  /// Frame range maximum nframes[0] * nframes[0]
  for(int k=min; k<max; k++) {
    ...
    for(int tid2=0; tid2 < toAggregate; tid2++) {
      ...
      /// Add up between different threads
      accH += acc[tid2][aidx].H.cast<double>();
    }

    /// The relative quantity becomes an absolute quantity through adj and is added to H, b
    ...
    b[tid].head<CPARS>().noalias() += accH.block<CPARS,1>(0,CPARS+8);   /// Residuals * intrinsic parameters
  }
