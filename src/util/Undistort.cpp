...
/* *******************************
 * @ function: photometric correction constructor
 *
 * @param: file response function parameter file
 * noiseImage
 * vignetteImage radiation attenuation image
 * w_, h_ image size
 *
 * @note:
****************************** */
    ...
    int w_, int h_)
{
  ...
  /// get the inverse transformation of the response function.
  {
    std::string line;
    std::getline( f, line );
    std::istringstream l1i( line );

    // begin iterator, end iterator to intialize
  ...
  /// if there is no calibration value, initialize it to 0-255 linear distribution value.
  if(setting_photometricCalibration==0) {
    for(int i=0;i<GDepth;i++) G[i]=255.0f*i/(float)(GDepth-1);
  }

  /// read the image, why read a 16-bit, and 8-bit?
...
/// add a response function to the image
void PhotometricUndistorter::unMapFloatImage(float* image) {
  ...
  for(int i=0; i<wh; i++) {
    ...
    /// large maximum
      ...
      BinvC=GDepth-1.1;
     /// interpolate the response function in the middle.
    else {
      int c = color;
      float a = color-c;
...
void PhotometricUndistorter::processFrame(T* image_in, float exposure_time, float factor)
{
  ...
  {
    /// remove the response function
    for(int i=0; i<wh;i++) {
      data[i] = G[image_in[i]];
    }

    /// remove the attenuation coefficient
...
// ******************************** Correction base class, including geometry and luminosity ******** ****************************
...
// read rectification parameters from a file.
Undistort* Undistort::getUndistorterForFile(std::string configFilename, std::string gammaFilename, std::string vignetteFilename)
{
  ...
  /// correct base class, as return value, other types inherit from it.
  Undistort* u;

  /// The following three specific models are for the model name that is not specified, only the parameters are given
  // for backwards-compatibility: Use RadTan model for 8 parameters.
  // If there are 8 parameters, it is a RadTan model
  ...
  // If it is five parameters, it is a pinhole or fov model
  ...
  /// Here are a few options that indicate the camera model
...
/// get the image w/o photometric params, and add geometric and photometric noise
// Correction, that is, packing images into ImageAndExposure class
template<typename T>
ImageAndExposure* Undistort::undistort(const MinimalImage<T>* image_raw, float exposure, double timestamp, float factor) const
{
  ...
  /// STEP1: remove the influence of photometric params
  photometricUndist->processFrame<T>(image_raw->data, exposure, factor);  /// remove the influence of photometric params
  ImageAndExposure* result = new ImageAndExposure(w, h, timestamp);
  photometricUndist->output->copyMetaTo(*result);                         /// only the exposure time is copied.

  /// \comment(edward): !passthrough is always working.
  if (!passthrough) {
    float* out_data = result->image;                      /// copy the image for output
    float* in_data = photometricUndist->output->image;    /// input image.

    /// STEP2: if the noise value is defined, set the random geometric noise size and add it to the output image
    float* noiseMapX=0;
    float* noiseMapY=0;
    ...
    for(int idx = w*h-1;idx>=0;idx--) {
      ...
      /// interpolate to get the output image with geometric noise
      if(xx<0) {
        out_data[idx] = 0;
      }
      else {
...
/// find the optimal camera internal params after correction
void Undistort::makeOptimalK_crop() {
  ...
  /// ? what does this mean with minX and maxX?
  for(int x=0; x<100000;x++) {
  ...
  while(oobLeft || oobRight || oobTop || oobBottom) {
    ...
    /// The X assignment is the largest and the smallest, and the Y assignment is mapped from small to large according to the coordinates between minY and minY
    /// ! This ensures that each Y coordinate corresponds to a maximum x and a minimum x, respectively, in order to find the boundary
    for(int y=0;y<h;y++) {
      remapX[y*2] = minX;
      remapX[y*2+1] = maxX;
    ...
    distortCoordinates(remapX, remapY,remapX, remapY,2*h); // Add distortion to the current image

    // If there are still not in the image range, continue to reduce
    for(int y=0;y<h;y++) {
      /// the smallest value is the left side to shrink
      if(!(remapX[2*y] > 0 && remapX[2*y] < wOrg-1))
        oobLeft = true;

      /// maximum value, that is, the right side is to be contracted.
      if(!(remapX[2*y+1] > 0 && remapX[2*y+1] < wOrg-1))
        oobRight = true;
    }

    /// ! Ensure that each X coordinate is the largest and smallest of the Y coordinate, and form the corresponding coordinate
    for(int x=0;x<w;x++) {
      remapY[x*2] = minY;
      remapY[x*2+1] = maxY;
    ...
    /// ! If it goes up and down, left and right, only the largest side is reduced
...
// @parameter: configuration file name, number of parameters, camera model name
void Undistort::readFromFile(const char* configFileName, int nPars, std::string prefix) {
  ...
  parsOrg = VecX(nPars);   /// camera original params
  ...
  /// fisrt line, camera model params, second line, camera pixel size.
  // l1 & l2
  if(nPars == 5) // fov model
  {
    char buf[1000];

    /// Copy prefix up to 1000, in% s format, to buf (char) array, %% means output%
    /// So buf is "fov% lf% lf% lf% lf% lf"
    snprintf(buf, 1000, "%s%%lf %%lf %%lf %%lf %%lf", prefix.c_str());

    /// Use buf for format control, output l1 to these 5 parameters
  ...
  /// cx, cy is less than 1, then it is a relative value, multiplied by the image size
  if(parsOrg[2] < 1 && parsOrg[3] < 1) {
    ...
    /// ?? 0.5 is still not very understandable, in order to use the integral to approximate the pixel intensity
  ...
  /// The third line, camera image category, whether to crop
// l3, third line, crop type
  if(l3 == "crop") {
  ...
  // * The fourth line, the size of the image, will be cropped according to the settings ...
  // l4 The fourth line, the resolution after cropping, put in the w, h variables
  ...
  /// l5 The fifth line, stores the baseline, and puts it in bl
  ...
  else if(outputCalibration[0] == -3)
  {
    ...
    /// Ratio to length and width (like TUMmono)
    K.setIdentity();
  ...
  /// Set fx and fy, then take the best
  ...
  /// * Calculate remapX and remapY for image correction
  for(int y=0;y<h;y++)
    for(int x=0;x<w;x++) {
      remapX[x+y*w] = x;
      remapY[x+y*w] = y;
...
// ********************* The following are algorithms for adding distortion ******************** ***********
